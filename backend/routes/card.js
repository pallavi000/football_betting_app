...
router.get("/archived/get", Authorization, async (req, res) => {
  try {
    ...
    if (req.user.role === "admin") {
      cards = await Card.find({ status: "archived" })
        .populate("matches")
        .sort("-_id");
    } else {
      cards = await UserCard.find({ status: "archived" })
        .populate("matches")
        .sort("-_id");
    }
    res.send(cards);
...
router.get("/:id", Authorization, async (req, res) => {
  try {
    const card = await Card.findById(req.params.id)
      .populate(["matches", "userCards", "winnersUsers"])
      .sort("-_id");
    res.send(card);
  } catch (error) {
    res.status(500).send(error.message);
...
async function updateResult(card, matches) {
  const users = [];
    ...
    for (let usermatch of userMatches) {
      if (match.result == usermatch.result) {
        const index = users.findIndex(
          (user) =>
            user.user_id.toString() == usermatch.user_id.toString() &&
            user.user_card_id.toString() == usermatch.user_card_id.toString()
        );
        if (index != -1) {
  ...
  for (const winner of winners) {
    ...
    await Winner.create({
      ...
      card_id: card._id,
      user_card_id: winner.user_card_id,
      reward: card.reward,
    });
    await UserCard.findByIdAndUpdate(winner.user_card_id, {
      winner: true,
      status: "winner",
    });
  }
...
router.put("/:id", Authorization, async (req, res) => {
  try {
    ...
    const cardWinners = await updateResult(card, req.body.results);
    card.winners = cardWinners;
    await card.save();

    await UserCard.updateMany(
      { card_id: req.params.id, status: "active" },
      {
        status: "archived",
      },
      { multi: true }
